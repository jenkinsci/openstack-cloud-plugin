...
public class JCloudsBuildWrapperTest {
    ...
    public void provisionSeveral() throws Exception {
        ...
        p.getBuildersList().add(new TestBuilder() {
            ...
            public boolean perform(AbstractBuild<?, ?> build, Launcher launcher, BuildListener listener)
                ...
                String[] ips = build.getEnvironment(TaskListener.NULL)
                        .get("JCLOUDS_IPS")
                        .split(",");
    ...
    public void failToProvisionWhenOpenstackFails() throws Exception {
        ...
        Server success = j.mockServer()
                .name("provisioned")
                .withFloatingIpv4("42.42.42.42")
                .get();
        ...
        when(network.getName()).thenReturn("netname");
        when(os.getNetworks(any())).thenReturn(Collections.singletonMap(networkId, network));

        FreeStyleProject p = j.createFreeStyleProject();
        List<InstancesToRun> instances =
                Collections.singletonList(new InstancesToRun(cloud.name, template.getName(), null, 2));
        p.getBuildWrappersList().add(new JCloudsBuildWrapper(instances));
