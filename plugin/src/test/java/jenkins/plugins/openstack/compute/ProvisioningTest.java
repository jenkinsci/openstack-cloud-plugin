...
public class ProvisioningTest {
    ...
    public void provisionSlaveOnDemand() throws Exception {
        ...
        SlaveOptions opts =
                j.defaultSlaveOptions().getBuilder().floatingIpPool("custom").build();
        ...
        assertEquals(
                "node:" + server.getName() + ":" + s.getId().getFingerprint(),
                server.getMetadata().get(ServerScope.METADATA_KEY));
    ...
    @Test
    @Issue("https://github.com/jenkinsci/openstack-cloud-plugin/issues/31")
    public void abortProvisioningWhenOpenstackFails() throws Exception {
        ...
        when(os.bootAndWaitActive(any(ServerCreateBuilder.class), anyInt()))
    ...
    @Test
    @Issue("https://github.com/jenkinsci/openstack-cloud-plugin/issues/37")
    public void detectBootTimingOut() {
        JCloudsSlaveTemplate template = j.dummySlaveTemplate("label");
        final JCloudsCloud cloud = j.dummyCloud(template);
        Openstack os = cloud.getOpenstack();
        Server server =
                j.mockServer().name("provisioned").status(Server.Status.BUILD).get();
    ...
    public void verifyOptionsPropagatedToLauncher() throws Exception {
        ...
        SlaveOptions expected = j.defaultSlaveOptions()
                .getBuilder()
        ...
        assertEquals(
                "java",
                launcher
                        .getJavaPath()); // https://github.com/jenkinsci/ssh-slaves-plugin/commit/9d25b12b1340e00d63a069f54c1b2361f745b6fc#commitcomment-49501649
    ...
    public void doProvision() throws Exception {
        JCloudsSlaveTemplate constrained = j.dummySlaveTemplate(
                j.defaultSlaveOptions().getBuilder().instanceCap(1).build(), "label");
        JCloudsSlaveTemplate free = j.dummySlaveTemplate("free");
        JCloudsCloud cloud = j.configureSlaveLaunchingWithFloatingIP(
                j.dummyCloud(j.defaultSlaveOptions().getBuilder().instanceCap(2).build(), constrained, free));
        cloud.setCleanfreq(60); // avoid cleaning
        ...
        assertThat(
                invokeProvisioning(cloud, wc, "/provision?name=no_such_template")
                        .getWebResponse()
                        .getContentAsString(),
        ...
        assertEquals(
                "node:" + server.getName() + ":" + slave.getId().getFingerprint(),
                server.getMetadata().get(ServerScope.METADATA_KEY));

        assertThat(
                invokeProvisioning(cloud, wc, "/provision?name=" + constrained.getName())
                        .getWebResponse()
                        .getContentAsString(),
        ...
        assertThat(
                invokeProvisioning(cloud, wc, "/provision?name=" + free.getName())
                        .getWebResponse()
                        .getContentAsString(),
    ...
    public void destroyTheServerWhenFipAllocationFails() throws Exception {
        SlaveOptions opts =
                j.defaultSlaveOptions().getBuilder().floatingIpPool("my_pool").build();
        ...
        when(os.assignFloatingIp(any(Server.class), any(String.class)))
                .thenThrow(new Openstack.ActionFailed("Unable to assign"));
    ...
    private void assertCloudMissingReportedOnce(CloudStatistics cs, AsyncResourceDisposer ard, JCloudsSlave foo) {
        ...
        final List<PhaseExecutionAttachment> attachments =
                activity.getCurrentPhaseExecution().getAttachments();
    ...
    public void timeoutLaunchingJnlp() throws Exception {
        final SlaveOptions opts =
                j.defaultSlaveOptions().getBuilder().startTimeout(10).build();
        final JCloudsCloud cloud =
                j.configureSlaveProvisioningWithFloatingIP(j.dummyCloud(opts, j.dummySlaveTemplate("asdf")));
        cloud.setCleanfreq(20); // 20 secondes > 7 sec for timeout
        final Iterable<NodeProvisioner.PlannedNode> pns =
                cloud.provision(new Cloud.CloudState(Label.get("asdf"), 0), 1);
    ...
    @Test
    @Issue("https://github.com/jenkinsci/openstack-cloud-plugin/issues/310")
    public void timeoutLaunchingSsh() throws Exception {
        LauncherFactory.SSH sshLaunch = new LauncherFactory.SSH("no-such-creds");
        final SlaveOptions opts = j.defaultSlaveOptions()
                .getBuilder()
                .startTimeout(3000)
                .launcherFactory(sshLaunch)
                .build();
        final JCloudsCloud cloud =
                j.configureSlaveProvisioningWithFloatingIP(j.dummyCloud(opts, j.dummySlaveTemplate("asdf")));
        ...
        // assertThat("Cause not fatal after ms " + aliveFor, computer.getFatalOfflineCause(),
    ...
    public void reportOfflineCauseInCloudStats() throws Exception {
        ...
        assertEquals(att.getTitle(), expected);
    ...
    */
    ...
    public void preferFloatingIpv4() throws Exception {
        verifyPreferredAddressUsed(
                "42.42.42.",
                Arrays.asList(
                        NetworkAddress.FIXED_4,
                        NetworkAddress.FIXED_6,
                        NetworkAddress.FLOATING_4,
                        NetworkAddress.FLOATING_6));
    }

    @Test
    public void preferFloatingIpv6() throws Exception {
        verifyPreferredAddressUsed(
                "4242:", Arrays.asList(NetworkAddress.FIXED_4, NetworkAddress.FIXED_6, NetworkAddress.FLOATING_6));
    }

    @Test
    public void preferFixedIpv4() throws Exception {
        verifyPreferredAddressUsed("43.43.43.", Arrays.asList(NetworkAddress.FIXED_6, NetworkAddress.FIXED_4));
    }

    @Test
    public void findFixedIpv4WhenNoExplicitTypeIsGiven() throws Exception {
        verifyPreferredAddressUsed("43.43.43.", List.of(NetworkAddress.FIXED_4_NO_EXPLICIT_TYPE));
    }
    ...
    private void verifyPreferredAddressUsed(String expectedAddress, Collection<NetworkAddress> addresses)
            throws Exception {
        CloudStatistics cs = CloudStatistics.get();
        ...
        assertThat(
                activity.getPhaseExecutions().toString(),
                activity.getCurrentPhase(),
                equalTo(ProvisioningActivity.Phase.OPERATING));

        Server server = cloud.getOpenstack().getServerById(computer.getNode().getServerId());
        assertEquals(
                "node:" + server.getName() + ":" + computer.getId().getFingerprint(),
                server.getMetadata().get(ServerScope.METADATA_KEY));
     ...
     */
    private static void waitForCloudStatistics(
            ProvisioningActivity activityToWaitFor, ProvisioningActivity.Phase expectedPhase)
        ...
        while (true) {
            ...
            final long timeToWaitNowInMilliseconds =
                    Math.min(millisecondsToWaitBetweenPolls, timeToWaitRemainingInMilliseconds);
    ...
    private static void waitForAsyncResourceDisposer() throws InterruptedException {
        ...
        while (true) {
            ...
            if (timeToWaitRemainingInMilliseconds <= 0L) {
                assertThat(
                        "After waiting " + timeSpentWaitingInMilliseconds + " milliseconds, "
                                + AsyncResourceDisposer.class.getSimpleName() + ".getBacklog()",
                        actual,
                        empty());
            }
            final long timeToWaitNowInMilliseconds =
